...
export const runSanitizeHtmlSpec = (environmentLabel: string) => {
  describe(`sanitizeHtml (${environmentLabel})`, () => {
    ...
    it("removes disallowed attributes while keeping allowed ones", () => {
      const result = sanitizeHtml(
        '<a href="https://example.com" style="color:red" ping="/x">go</a>'
      );
    ...
    it("preserves tables and images", () => {
      ...
      // Void-tag serialization is the one permitted divergence between the
      // environments: sanitize-html (server) emits `<img ... />`, DOMPurify
      // (client) emits `<img ...>`. Browsers parse both identically, so
      // hydration is unaffected; content and attributes must still match.
      expect(result).toMatch(/<img src="\/photo\.jpg" alt="Town hall" ?\/?>/);
    });

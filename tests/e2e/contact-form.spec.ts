...
test.describe("Generic Contact Form", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // The CMS catch-all serves 200 HTML for unknown routes, so a non-404
    // status can't prove the generic form is mounted — skip unless its
    // distinctive single "Name" field actually renders.
    try {
      await page
        .locator("form")
        .getByLabel(/^name$/i)
      ...
      test.skip(
        true,
        `${CONTACT_URL} does not render the generic ContactForm (catch-all page served instead)`
      );
    }
  });

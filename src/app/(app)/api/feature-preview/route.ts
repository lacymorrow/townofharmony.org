...
import { env } from "@/env";
import { type FeatureFlagName, FLAGS, PREVIEW_COOKIE } from "@/lib/preview-flags";

// LAC-2963 (CWE-601): only same-origin paths may pass through ?redirect=.
// "//host" and "/\host" are rejected up front because new URL() resolves both
// as protocol-relative; the origin comparison backstops any other parser quirk.
function resolveRedirect(raw: string | null, requestUrl: string): URL {
...
export async function GET(request: Request) {
  ...
  const redirectTo = resolveRedirect(searchParams.get("redirect"), request.url);
